// Default data + helpers for the editable app.
// STATE shape (lives in app.jsx):
//   steps:   [{ id, phase, task, who }]            — global; editable
//   whoPalette: { [name]: { bg, tx, dot } }        — global; can grow with custom names
//   videos:  [{ id, name, deadline, archived, checked: { [stepId]: true }, createdAt }]
//   activeId, viewArchive, openWhoStepId, openVideoMenuFor
...
const DEFAULT_WHO_PALETTE = {
  ...
  'Claude':           { bg: '#ffe7d6', tx: '#b85c1f', dot: '#e88a4a' },
...
// Color pool for newly-created performers. Picks the next unused hue.
const PERFORMER_COLOR_POOL = [
  { bg: '#e0e9ff', tx: '#3b4d99', dot: '#6b85d6' }, // soft blue
  { bg: '#ffe2e7', tx: '#a83454', dot: '#e36a85' }, // soft red
  { bg: '#dff2d9', tx: '#3d7530', dot: '#7ab560' }, // soft leaf green
  { bg: '#fff0d6', tx: '#a06420', dot: '#d99540' }, // amber
  { bg: '#e6e1f5', tx: '#5c4a99', dot: '#8b78c4' }, // lavender
  { bg: '#dcefef', tx: '#266866', dot: '#4ea4a0' }, // teal
  { bg: '#f3e0ef', tx: '#8c3877', dot: '#c168a9' }, // pink-violet
  { bg: '#eaeae0', tx: '#5a5836', dot: '#9a956b' }, // moss
];
...
// Iconography for phases (geometric glyphs, no emoji)
...
function appPhaseProgress(video, steps, phase) {
  ...
  return { done, total: inPhase.length, pct: inPhase.length ? Math.round(done / inPhase.length * 100) : 0 };
}
...
// Initial state factory (used when nothing is in localStorage / firebase)
function appInitialState() {
  return {
    ...
    videos: [
      { id: 1, name: 'Видео 1', deadline: null, archived: false, checked: {}, createdAt: Date.now() },
    ],
  };
}

// Migration helper — accepts the old shape from existing index.html (videos only).
function appMigrate(raw) {
  // raw can be: array of videos (old format), or full state object (new format)
  ...
  if (raw && typeof raw === 'object') {
    ...
    return {
      ...
      videos: Array.isArray(raw.videos) && raw.videos.length ? raw.videos : base.videos,
    };
  }
