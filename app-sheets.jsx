          ...
          {names.map(n => {
            ...
            return (
              <button key={n} onClick={() => onPick(n)} style={{
                ...
                {isCurrent && (<svg width="18" height="14" viewBox="0 0 14 11" fill="none"><path d="M1 5.5L5 9.5L13 1.5" stroke="#8b5e3c" strokeWidth="2.3" strokeLinecap="round" strokeLinejoin="round" /></svg>)}
              </button>
            );
          })}
        </div>
          ...
          {adding ? (
            <div style={sheetStyles.addBox}>
              <input autoFocus value={name} onChange={e => setName(e.target.value)}
                ...
                placeholder="Например: Монтажёр Саша" style={sheetStyles.input} />
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '4px 16px 32px' }}>
        {mode === 'rename' ? (
          ...
          <div style={sheetStyles.addBox}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
              <button style={{ ...sheetStyles.btn, ...sheetStyles.btnGhost }} onClick={() => setMode(null)}>Назад</button>
              {video.deadline && (<button style={{ ...sheetStyles.btn, ...sheetStyles.btnGhost }} onClick={() => { onSetDeadline(null); onClose(); }}>Убрать</button>)}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            {video.archived ? (
              ...
              <MenuItem icon="🗄" label="Убрать в архив" onClick={() => { onArchive(); onClose(); }} />
            )}
...
const sheetStyles = {
  ...
  x: { width: 32, height: 32, borderRadius: 16, border: 'none', background: '#ece2d2', color: '#7a6048', fontFamily: 'inherit', fontSize: 14, cursor: 'pointer', flexShrink: 0 },
  whoRow: { display: 'flex', alignItems: 'center', gap: 12, padding: '14px 14px', borderRadius: 14, border: '1.5px solid #ece2d2', background: '#fffdf8', cursor: 'pointer', fontFamily: 'inherit', minHeight: 56 },
  addBtn: { width: '100%', padding: '14px', borderRadius: 14, background: 'transparent', border: '1.5px dashed #c4a882', color: '#8b5e3c', fontFamily: 'inherit', fontSize: 15, fontWeight: 600, cursor: 'pointer' },
  addBox: { padding: 14, borderRadius: 14, background: '#fffdf8', border: '1px solid #ece2d2' },
  input: { width: '100%', padding: '12px 14px', borderRadius: 10, border: '1.5px solid #d0c0a8', background: '#fff', fontSize: 16, fontFamily: 'inherit', color: '#2a2420', outline: 'none' },
...
const archStyles = {
  ...
  backBtn: { display: 'inline-flex', alignItems: 'center', gap: 6, background: 'transparent', border: 'none', color: '#8b5e3c', fontSize: 14, fontFamily: 'inherit', fontWeight: 600, cursor: 'pointer', padding: '4px 0', marginBottom: 4 },
  ...
  card: { display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', marginBottom: 10, borderRadius: 16, background: '#fffdf8', border: '1px solid #ece2d2' },
  restoreBtn: { background: '#f0e8d8', border: 'none', borderRadius: 10, padding: '8px 14px', fontSize: 13, fontWeight: 600, color: '#6b4423', cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 },
};
