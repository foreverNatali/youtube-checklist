// Main App component — carousel + checklist + archive
...
function App({ initialState, onPersist, syncLabel, user, onLogin, onLogout, fbReady, fbError }) {
  ...
  React.useEffect(() => { onPersist(state); }, [state, onPersist]);
  ...
  const pickWho = (who) => { updateStep(whoStepId, { who }); setWhoStepId(null); };
  ...
  if (view === 'archive') {
    return (
      <div style={appStyles.root}>
        <ArchiveView videos={archivedVideos} steps={state.steps} palette={state.whoPalette}
          onUnarchive={unarchiveVideo} onDelete={deleteVideo}
          onOpen={(vid) => { setActiveId(vid); setView('live'); }} onBack={() => setView('live')} />
        {videoMenuId && (
          <VideoMenuSheet
  ...
  return (
    <div style={appStyles.root}>
      ...
      {!user && (
        <div style={appStyles.signInBanner}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#6b4423', marginBottom: 2 }}>Войди чтобы видеть свои видео</div>
            <div style={{ fontSize: 11.5, color: '#8b7256', lineHeight: 1.35 }}>
              {fbError ? `Ошибка: ${fbError}` : fbReady ? 'Синхро через Google аккаунт' : 'Подключаюсь к серверу…'}
            </div>
          </div>
      ...
      {liveVideos.length === 0 ? (
        ...
        <>
          <div ref={scrollerRef} style={appStyles.scroller}>
            ...
            {liveVideos.map(v => (
              <VideoCard key={v.id} v={v} steps={state.steps} palette={state.whoPalette}
                isActive={v.id === activeId}
                cardRef={el => (cardRefs.current[v.id] = el)}
                onTap={() => {
                  if (v.id === activeId) setVideoMenuId(v.id);
                  else cardRefs.current[v.id]?.scrollIntoView?.({ behavior: 'smooth', inline: 'center', block: 'nearest' });
                }}
          ...
          <div style={appStyles.dots}>
            {liveVideos.map(v => (<div key={v.id} style={{ ...appStyles.dot, ...(v.id === activeId ? appStyles.dotActive : null) }} />))}
          </div>
        </>
      )}
        ...
        <div style={appStyles.checklist}>
          {isAllDone ? (
            <div style={appStyles.doneBlock}>
              ...
              <button style={appStyles.archiveBigBtn} onClick={() => archiveVideo(active.id)}>🗄 Убрать в архив</button>
            </div>
          ) : (
            state.phases.map(phase => {
              ...
              return (
                <div key={phase} style={{ marginBottom: 16 }}>
                  ...
                  {inPhase.map(step => (
                    <TaskRow key={step.id} step={step} isDone={!!active.checked[step.id]}
                      palette={state.whoPalette} onToggle={() => toggle(step.id)} onPickWho={() => setWhoStepId(step.id)} />
                  ))}
                </div>
              );
            })
      ...
      {whoStepId && (
        <WhoPickerSheet step={state.steps.find(s => s.id === whoStepId)}
          palette={state.whoPalette} onPick={pickWho} onAddNew={addNewPerformer}
          onClose={() => setWhoStepId(null)} />
      )}
      {videoMenuId && state.videos.find(v => v.id === videoMenuId) && (
        <VideoMenuSheet video={state.videos.find(v => v.id === videoMenuId)}
          onClose={() => setVideoMenuId(null)}
          onRename={(name) => updateVideo(videoMenuId, { name })}
          onSetDeadline={(d) => updateVideo(videoMenuId, { deadline: d })}
          onArchive={() => archiveVideo(videoMenuId)}
          onUnarchive={() => unarchiveVideo(videoMenuId)}
          onDelete={() => deleteVideo(videoMenuId)}
          isAllDone={(() => { const v = state.videos.find(v => v.id === videoMenuId); return v ? appProgress(v, state.steps).done === state.steps.length : false; })()} />
      )}
    </div>
  );
}
...
function VideoCard({ v, steps, palette, isActive, onTap, onMenu, cardRef }) {
  ...
  return (
    <div ref={cardRef} onClick={onTap} style={{ ...appStyles.card, ...(isActive ? appStyles.cardActive : null) }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {isActive && (
            <button onClick={(e) => { e.stopPropagation(); onMenu(); }} style={appStyles.menuBtn} title="Меню">
      ...
      <div style={appStyles.cardBottom}>
        <ProgressRing pct={p.pct} done={p.done} total={p.total} />
        <div style={{ flex: 1, minWidth: 0 }}>
          {allDone ? (<div style={appStyles.cardNextDone}>Готово к архиву ✓</div>) : (
            <>
...
function TaskRow({ step, isDone, palette, onToggle, onPickWho }) {
  ...
  return (
    <div style={{ ...appStyles.taskRow, ...(isDone ? appStyles.taskRowDone : null) }}>
      <button onClick={onToggle} style={appStyles.checkBtn}>
        <div style={{ ...appStyles.checkbox, ...(isDone ? appStyles.checkboxDone : null) }}>
          {isDone && (<svg width="14" height="11" viewBox="0 0 14 11" fill="none"><path d="M1 5.5L5 9.5L13 1.5" stroke="#fff" strokeWidth="2.3" strokeLinecap="round" strokeLinejoin="round" /></svg>)}
        </div>
      </button>
      <div onClick={onToggle} style={{ flex: 1, minWidth: 0, cursor: 'pointer' }}>
        <div style={{ ...appStyles.taskText, textDecoration: isDone ? 'line-through' : 'none', color: isDone ? '#b0a090' : '#2a2420' }}>{step.task}</div>
      </div>
      <button onClick={(e) => { e.stopPropagation(); onPickWho(); }} style={{ ...appStyles.whoChip, background: who.bg, color: who.tx }}>{step.who}</button>
    </div>
  );
}
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
        ...
        <circle cx={size/2} cy={size/2} r={r} stroke={allDone ? '#3aa978' : '#c4956a'} strokeWidth="3" fill="none"
          strokeDasharray={C} strokeDashoffset={off} strokeLinecap="round" style={{ transition: 'stroke-dashoffset .35s' }} />
      </svg>
      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center',
        fontFamily: 'inherit', fontSize: 12.5, fontWeight: 700, color: allDone ? '#3aa978' : '#6b4423' }}>{allDone ? '✓' : `${done}/${total}`}</div>
    </div>
  );
}
...
const appStyles = {
  ...
  iconBtn: { position: 'relative', width: 40, height: 40, borderRadius: 20, border: '1px solid #d8c8b0', background: '#fffdf8', color: '#6b4423', cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center' },
  iconBadge: { position: 'absolute', top: -4, right: -4, minWidth: 16, height: 16, padding: '0 4px', borderRadius: 8, background: '#8b5e3c', color: '#fffdf8', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1.5px solid #f5f0e8' },
  syncLine: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8, minHeight: 18 },
  linkBtn: { background: 'transparent', border: 'none', color: '#8b5e3c', fontSize: 12, fontFamily: 'inherit', fontWeight: 600, cursor: 'pointer', padding: 0 },
  signInBanner: { display: 'flex', alignItems: 'center', gap: 12, margin: '4px 20px 0', padding: '12px 14px', background: '#fff7e8', border: '1px solid #e8c890', borderRadius: 14 },
  signInBtn: { background: '#8b5e3c', color: '#fffdf8', border: 'none', borderRadius: 10, padding: '10px 16px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0 },
  empty: { textAlign: 'center', padding: '60px 20px' },
  scroller: { display: 'flex', gap: 14, overflowX: 'auto', overflowY: 'visible', scrollSnapType: 'x mandatory', WebkitOverflowScrolling: 'touch', paddingTop: 14, paddingBottom: 8, scrollbarWidth: 'none' },
  card: { flex: '0 0 280px', scrollSnapAlign: 'center', minWidth: 0, background: '#fffdf8', borderRadius: 20, padding: '14px 16px 16px', border: '1px solid #e9ddc8', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column', gap: 11, cursor: 'pointer', transition: 'transform .2s, box-shadow .2s, border-color .2s', minHeight: 160 },
  ...
  menuBtn: { background: 'transparent', border: 'none', padding: 6, borderRadius: 6, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: -4 },
  ...
  phaseHead: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 11, letterSpacing: 1.5, fontWeight: 700, textTransform: 'uppercase', padding: '14px 4px 8px' },
  taskRow: { display: 'flex', alignItems: 'center', gap: 10, padding: '12px 12px', marginBottom: 8, borderRadius: 14, background: '#fffdf8', border: '1px solid #ece2d2', minHeight: 56 },
  ...
  whoChip: { fontSize: 10.5, fontWeight: 600, padding: '6px 9px', borderRadius: 12, flexShrink: 0, whiteSpace: 'nowrap', letterSpacing: 0.1, border: 'none', cursor: 'pointer', fontFamily: 'inherit', minHeight: 28 },
  doneBlock: { textAlign: 'center', padding: '32px 20px 28px', background: '#fff7e8', borderRadius: 18, border: '1px solid #e8c890', margin: '12px 0' },
  archiveBigBtn: { background: '#6b4423', color: '#fffdf8', border: 'none', borderRadius: 12, padding: '12px 22px', fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', display: 'inline-flex', alignItems: 'center' },
};
